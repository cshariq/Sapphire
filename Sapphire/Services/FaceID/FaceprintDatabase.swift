...
class FaceDataStore {
    ...
    private let depthDetector = DepthDetector()
    ...
    private func setupSettingsObserver() {
        SettingsModel.shared.$settings
            ...
            .sink { [weak self] hasRegistered in
                guard let self = self, !hasRegistered, self.hasRegisteredFaceprints() else { return }
                DispatchQueue.main.async { SettingsModel.shared.settings.hasRegisteredFaceID = true }
            }
            .store(in: &cancellables)
    }

    func getRegisteredProfileNames() -> [String] { profiles.keys.sorted() }
    ...
    func hasRegisteredFaceprints() -> Bool { !profiles.isEmpty }
    func reset() {
        profiles.removeAll()
        depthDetector.reset()
        try? FileManager.default.removeItem(at: secureFileURL)
        print("️ All face recognition data has been reset.")
        DispatchQueue.main.async { SettingsModel.shared.settings.hasRegisteredFaceID = false }
    }
    func register(faceSamples: [(observation: VNFaceObservation, buffer: CVPixelBuffer)], forProfile name: String) {
        ...
        newDatabase.individualPrints = faceSamples.compactMap { generateEmbedding(for: $0.observation, from: $0.buffer) }
        ...
        if let globalAvg = average(faceprints: newDatabase.individualPrints) { newDatabase.globalAverage = globalAvg }
    ...
    func learnNewFaceprint(faceImage: NSImage) {
        guard let profileName = profiles.keys.first, var database = profiles[profileName], let newEmbedding = generateEmbedding(for: faceImage) else { return }
    ...
    private func saveToSecureStorage() {
        ...
        } catch { print(" Failed to save profiles to secure storage: \(error)") }
    }
    private func loadFromSecureStorage() {
        ...
        do {
            ...
            if let firstDb = profiles.values.first { depthDetector.loadDepthMaps(from: firstDb.serializedDepthMaps) }
            print(" Secure faceprint profiles loaded successfully.")
        } catch { print(" Error loading secure profiles: \(error)") }
    }
    ...
    private func generateEmbedding(for image: NSImage) -> [Float]? {
        guard let arcFaceModel = MLModelManager.shared.arcFaceModel else { print("ERROR (ML): ArcFace model is not loaded."); return nil }
        guard let mlArray = preprocess(image: image) else { return nil }
        do {
            let modelInput = ArcFaceInput(x_1: mlArray)
            let output = try arcFaceModel.prediction(input: modelInput)
            let vector = (0..<output.var_657.count).map { output.var_657[$0].floatValue }
            return l2Normalize(vector)
        } catch { print(" Failed to get prediction from ArcFace model: \(error)"); return nil }
    }
    private func preprocess(image: NSImage) -> MLMultiArray? {
        let targetSize = CGSize(width: 112, height: 112)
        guard let cgImage = image.cgImage else { return nil }
        let resizedImage = NSImage(size: targetSize)
        resizedImage.lockFocus(); NSGraphicsContext.current?.imageInterpolation = .high
        let targetRect = CGRect(origin: .zero, size: targetSize)
        NSGraphicsContext.current?.cgContext.draw(cgImage, in: targetRect); resizedImage.unlockFocus()
        ...
        guard let context = CGContext(data: nil, width: width, height: height, bitsPerComponent: 8, bytesPerRow: width * 4, space: CGColorSpaceCreateDeviceRGB(), bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue), let pixelData = context.data else { return nil }
        ...
        for y in 0..<height {
            for x in 0..<width {
                ...
                let r = (Float(ptr[offset]) - 127.5) / 128.0, g = (Float(ptr[offset+1]) - 127.5) / 128.0, b = (Float(ptr[offset+2]) - 127.5) / 128.0
    ...
    private func l2Normalize(_ vector: [Float]) -> [Float] {
        let norm = sqrt(vector.map { $0 * $0 }.reduce(0, +)); guard norm > 0 else { return vector }; return vector.map { $0 / norm }
    }
    private func cosineSimilarity(_ a: [Float], _ b: [Float]) -> Float {
        guard a.count == b.count, !a.isEmpty else { return 0 }; return zip(a, b).map(*).reduce(0, +)
    }
    private func average(faceprints: [[Float]]) -> [Float]? {
        guard let first = faceprints.first, !first.isEmpty else { return nil }; var avg = [Float](repeating: 0.0, count: first.count)
        for print in faceprints { for i in 0..<first.count { avg[i] += print[i] } }
        return l2Normalize(avg.map { $0 / Float(faceprints.count) })
    }
}