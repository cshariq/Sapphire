...
class MusicManager: ObservableObject {
    ...
    private func setupHandlers() {
        mediaController.onTrackInfoReceived = { [weak self] trackInfo in
            Task { @MainActor in
                ...
                let sourceBundleID = payload.bundleIdentifier ?? "N/A"
    ...
    func openInSourceApp() {
        if lastKnownBundleID == "com.apple.Music" { appleMusic.revealCurrentTrack() }
        else if let uriString = self.uri, let url = URL(string: uriString) { NSWorkspace.shared.open(url) }
    }
