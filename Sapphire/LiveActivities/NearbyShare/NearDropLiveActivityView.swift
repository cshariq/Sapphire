...
private struct IntegratedActionIconButtonsView: View {
    ...
    private func submitConsent(accept: Bool, action: NearDropUserAction = .save) {
        guard accept else {
            liveActivityManager.declineNearDropTransfer(id: payload.id)
            return
        }
        liveActivityManager.updateNearDropState(to: .inProgress)
        NearbyConnectionManager.shared.submitUserConsent(transferID: payload.id, accept: true, action: action)
    }

    var body: some View {
        HStack(spacing: 10) {
            let declineButton = Button { submitConsent(accept: false) } label: { Image(systemName: "xmark") }
                .buttonStyle(CircleIconButtonStyle(type: .destructive))
            ...
            if let textContent = payload.transfer.textDescription {
                if let _ = URL(string: textContent) {
                    ...
                    Button { submitConsent(accept: true, action: .copy) } label: { Image(systemName: "doc.on.doc") }
                        .buttonStyle(CircleIconButtonStyle(type: .prominent))
