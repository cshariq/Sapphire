...
class WebSocketManager: NSObject, URLSessionWebSocketDelegate, URLSessionDelegate {
    ...
    func urlSession(_ session: URLSession, webSocketTask: URLSessionWebSocketTask, didOpenWithProtocol protocol: String?) {
        ...
        print("[WebSocketManager] WebSocket Handshake SUCCESSFUL.")
        schedulePing()
    }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if let error = error {
            ...
            print("[WebSocketManager] WebSocket Task COMPLETED CLEANLY.")
        }
    }
                ...
                self.webSocketTask?.send(.string("{\"type\":\"ping\"}")) { error in
                    if let error = error {
                        ...
                        self.handleConnectionFailure()
                         ...
                         print("[WebSocketManager] Ping sent successfully.")
                    }
                }
            }
        }
    ...
    private func handleConnectionFailure() {
        print("[WebSocketManager] Connection failure handler triggered.")
        reconnect(delay: 5.0)
    }
