...
struct AlbumArtView: View {
    ...
    var body: some View {
        Group {
            if let artwork = image { Image(nsImage: artwork).resizable() }
            else { Image(systemName: "music.note").foregroundColor(.white.opacity(0.8)) }
        }.aspectRatio(contentMode: .fill).frame(width: 20, height: 20).clipShape(RoundedRectangle(cornerRadius: 7, style: .continuous)).id(image)
    }
}
...
struct BatteryActivityView { static func left(for state: BatteryState) -> some View { Image(systemName: state.isLow ? "battery.25" : (state.isCharging ? "bolt.fill" : "powerplug.fill")).frame(width: 20, height: 20).foregroundColor(state.isLow ? .red : (state.isCharging ? .green : .white.opacity(0.9))).background(Color.white.opacity(0.1)).clipShape(RoundedRectangle(cornerRadius: 6, style: .continuous)).shadow(radius: 2) }; static func right(for state: BatteryState) -> some View { Text("\(state.level)%").font(.system(size: 13, weight: .semibold)).foregroundColor(state.isLow ? .red.opacity(0.95) : .white.opacity(0.9)) } }
struct BatteryRingView: View { let level: Int; private var color: Color { if level <= 20 { .red } else if level <= 45 { .yellow } else { .green } }; var body: some View { ZStack { Circle().stroke(Color.white.opacity(0.2), lineWidth: 2.5); Circle().trim(from: 0, to: CGFloat(level) / 100.0).stroke(color, style: StrokeStyle(lineWidth: 2.5, lineCap: .round)).rotationEffect(.degrees(-90)).animation(.easeOut, value: level) }.frame(width: 18, height: 18) } }
struct BluetoothActivityView { static func left(for device: BluetoothDeviceState) -> some View { Image(systemName: device.iconName).font(.system(size: 18, weight: .semibold)).foregroundColor(.white).symbolRenderingMode(.hierarchical) }; static func right(for device: BluetoothDeviceState) -> some View { HStack(spacing: 8) { Text(device.name).font(.system(size: 13, weight: .semibold)).foregroundColor(.white.opacity(0.9)); if let level = device.batteryLevel { BatteryRingView(level: level) } } } }
...
struct BluetoothDisconnectedActivityView { static func left(for device: BluetoothDeviceState) -> some View { Image(systemName: "wifi.slash").font(.system(size: 18, weight: .semibold)).foregroundColor(.white.opacity(0.8)).symbolRenderingMode(.hierarchical) }; static func right(for device: BluetoothDeviceState) -> some View { VStack(alignment: .leading, spacing: 1) { Text(device.name).font(.system(size: 13, weight: .semibold)).foregroundColor(.white.opacity(0.9)).lineLimit(1); Text("Disconnected").font(.system(size: 11)).foregroundColor(.white.opacity(0.6)) } } }
struct CalendarActivityView { static func left(for event: EKEvent) -> some View { Image(systemName: "calendar").resizable().aspectRatio(contentMode: .fit).frame(width: 18, height: 18).foregroundColor(.white.opacity(0.85)).padding(.vertical, 2) }; static func right(for event: EKEvent) -> some View { VStack(alignment: .leading, spacing: 2) { Text(event.title).lineLimit(1).font(.system(size: 13, weight: .semibold)); Text(event.startDate, style: .relative).lineLimit(1).font(.system(size: 11, weight: .medium)).foregroundColor(.white.opacity(0.6)) } } }
