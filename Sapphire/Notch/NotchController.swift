...
struct NotchController: View {
    ...
    private func buildLeftView(for data: StandardActivityData) -> some View {
        ...
        case .battery(let state, let style, let timeRemaining):
            switch style {
            case .persistent: PersistentBatteryActivityView.left(for: state, timeRemaining: timeRemaining)
            case .default: DefaultBatteryActivityView.left(for: state)
            case .compact: CompactBatteryActivityView.left(for: state)
            }
    ...
    private func buildRightView(for data: StandardActivityData) -> some View {
        ...
        case .battery(let state, let style, let timeRemaining):
            switch style {
            case .persistent: PersistentBatteryActivityView.right(for: state)
            case .default: DefaultBatteryActivityView.right(for: state, timeRemaining: timeRemaining)
    ...
    public func handleItemDrop(providers: [NSItemProvider]) -> Bool {
        ...
        return false // Actual handling likely happens elsewhere
    }
