...
struct OnboardingView: View {
    enum OnboardingStep {
        case welcome, permissions, privacyPolicy, musicChoice, spotifySetup, corePreferences, lockScreenSetup, finish
    }
    ...
    var body: some View {
        ZStack {
            ...
            ZStack {
                ...
                case .permissions:
                    PermissionsStepView(onContinue: { currentStep = .privacyPolicy })
                        .transition(.asymmetric(insertion: .move(edge: .trailing), removal: .move(edge: .leading)).combined(with: .opacity))
                case .privacyPolicy:
...
// MARK: - Step 8: Finish (Merged View)
